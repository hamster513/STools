        ...
        for file_path in file_list:
            ...
            try:
                ...
                if 'epss' in file_path.lower() and file_path.endswith('.csv.gz'):
                    ...
                    if len(lines) > 0:
                        print(f"📝 Первая строка (метаданные): {lines[0][:200]}")
                    if len(lines) > 1:
                        print(f"📝 Вторая строка (заголовок): {lines[1][:200]}")
                    if len(lines) > 2:
                        print(f"📝 Третья строка (данные): {lines[2][:200]}")
                    
                    # Пропускаем первую строку с метаданными (начинается с #)
                    if lines and lines[0].startswith('#'):
                        print(f"🔧 Пропускаем первую строку с метаданными")
                        lines = lines[1:]
